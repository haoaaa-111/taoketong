# Session 09 — Onboarding Step 2（用户画像问卷）

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers/executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 完成 Onboarding Step 2，实现用户画像问卷 UI，收集逃课偏好和约束条件。

**Architecture:** 单选组件从 localStorage 读取 Step 1 数据。问卷通过后写入 `/api/profile` 和 `/api/config`，跳转到 Step 3。

**Tech Stack:** React, TailwindCSS, Next.js App Router

**Source docs:**
- `design-spec.md` Section 5.2 Step 2（用户画像问卷）
- `design-spec.md` Section 二（UserProfile, UserConfig 字段定义）

**前置依赖:** Session 08（onboarding 流程 + localStorage 传递）
...
| 文件 | 操作 | 职责 |
|------|------|------|
| `src/app/onboarding/step2/page.tsx` | 创建 | Step 2 页面 + 问卷逻辑 |
| `src/components/onboarding/ChipSelect.tsx` | 创建 | 多选 chips 组件 |

---

### Task 1: ChipSelect 组件

- [ ] **Step 1: 创建 `src/components/onboarding/ChipSelect.tsx`**
...
- [ ] **Step 1: 创建目录 + `src/app/onboarding/step2/page.tsx`**

```bash
mkdir -p src/app/onboarding/step2
```
...
- [ ] **Step 2: 流程验证** — `npm run dev`，从 `/onboarding/step1` 完成 → 应该跳转到 Step 2，表单提交后跳转到 Step 3（404 正常，下一步实现）。
- [ ] **Step 3: 提交**

```bash
git add -A
git commit -m "feat: Onboarding Step 2（用户画像问卷）"
```

---

> **下一 Session:** Session 10 — Onboarding Step 3（课程校对）
